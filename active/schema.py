...
class VNSeqMByIterations(BaseSchema):
    """
    Add views spaced evenly across the total training iterations.
    """

    def __init__(self, scene, total_iterations: int, N: int = 20, M: int = 1, num_init_views: int = 4, **kwargs):
        """
        N: total number of views to use
        M: number of views to add at each stage
        total_iterations: total number of training iterations
        """
        super().__init__()

        assert N >= num_init_views, "Total views N must be at least the number of initial views"
        assert (N - num_init_views) % M == 0, "(N - num_init_views) must be divisible by M"

        self.init_views = [0]  # initial seed view
        self.load_its = {}

        # Select (num_init_views - 1) most distant views
        all_cams = scene.train_camera
        candidate_views = [i for i in range(len(all_cams)) if i not in self.init_views]
        train_cams = [all_cams[i] for i in self.init_views]
        candidate_cams = [all_cams[i] for i in candidate_views]
        ...
        selected_idxs = []
        for _ in range(num_init_views - len(self.init_views)):
            trainT = torch.stack([i.camera_center.cpu() for i in train_cams])
            candidateT = torch.stack([i.camera_center.cpu() for i in candidate_cams])
            dist_mat = torch.cdist(candidateT, trainT)
            candidate_dist = reduce(dist_mat, "c t -> c", "min")
        ...
        # Schedule view additions evenly across training
        num_view_adds = (N - len(self.init_views)) // M
        interval = total_iterations // (num_view_adds + 1)

        cur_dataset_size = len(self.init_views)
        for i in range(num_view_adds):
            iteration = interval * (i + 1)
            self.load_its[iteration] = M
            cur_dataset_size += M
...
V20Seq4Inplace = partial(VNSeqMInplace, N=240, M=4, num_init_views=4, interval_epochs=300)

coarse = partial(VNSeqMInplace, N=240, M=4, num_init_views=4, interval_epochs=300)
